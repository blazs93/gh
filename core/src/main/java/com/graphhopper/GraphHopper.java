...
public class GraphHopper implements GraphHopperAPI {
	...
	protected DataReader initDataReader(DataReader reader) {
		...
		return reader.setFile(new File(dataReaderFile)).setElevationProvider(eleProvider)
				.setWorkerThreads(dataReaderWorkerThreads).setWayPointMaxDistance(dataReaderWayPointMaxDistance);
	}
	...
	public List<Path> calcPaths(GHRequest request, GHResponse ghRsp) {
		...
		try {
			...
			for (int i = 0; i < maxRetries; i++) {
				...
				PathDetailsBuilderFactory calculatorFactory = new PathDetailsBuilderFactory(request.getPathDetails(),
						encoder);

				DouglasPeucker peucker = new DouglasPeucker().setMaxDistance(wayPointMaxDistance);
				PathMerger pathMerger = new PathMerger().setCalcPoints(tmpCalcPoints).setDouglasPeucker(peucker)
						.setEnableInstructions(tmpEnableInstructions).setPathDetailsBuilderFactory(calculatorFactory)
	...
	public void setLmFactoryDecoratorWeightingList(final List<String> weightingList) {

		lmFactoryDecorator.setWeightingsAsStrings(weightingList);
	}
